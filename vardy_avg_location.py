...
def plot_player(x,y,z,a):
    ...
    player_events = picked_game[picked_game.player == x]
    player1_events = picked_game[picked_game.player == z]
    #player2_events = picked_game[picked_game.player == e]
    #player3_events = picked_game[picked_game.player == f]
    #player4_events = picked_game[picked_game.player == g]
    #player1_events = picked_game[picked_game.player == z]
   ...
   # player3_events[['loc_x', 'loc_y']] = pd.DataFrame(player3_events.location.tolist(), index=player3_events.index)
    ...
    plots = pitch.scatter(avg_loc_x, avg_loc_y, ax=ax, color= y, s=200)
    pitch.annotate(text='Vardy', xy=(avg_loc_x-3,avg_loc_y+6), xytext=None, ax=ax, color='white')
    
    plots1 = pitch.scatter(120-avg1_loc_x, avg1_loc_y, ax=ax, color= a, s=200)
    pitch.annotate(text='Coates', xy=(120- avg1_loc_x-3,avg1_loc_y+6), xytext=None, ax=ax, color='white')
    
    #plots2 = pitch.scatter(120-avg2_loc_x, avg2_loc_y, ax=ax, color= f, s=200)
    #pitch.annotate(text='Coates', xy=(120- avg2_loc_x-3,avg2_loc_y+6), xytext=None, ax=ax, color='white')
    
    #plots3 = pitch.scatter(120-avg3_loc_x, avg3_loc_y, ax=ax, color= a, s=200)
    #pitch.annotate(text='Coates', xy=(120- avg3_loc_x-3,avg3_loc_y+6), xytext=None, ax=ax, color='white')
    
   # plots4 = pitch.scatter(120-avg4_loc_x, avg4_loc_y, ax=ax, color= a, s=200)
    #pitch.annotate(text='Coates', xy=(120- avg4_loc_x-3,avg4_loc_y+6), xytext=None, ax=ax, color='white')
    return plots, plots1
